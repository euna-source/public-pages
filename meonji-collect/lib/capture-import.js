...
export async function importCapture(bundle, adapter = new LocalAdapter()) {
  ...
  for (const item of bundle.items) {
    ...
    const note = [item.brand && `브랜드: ${item.brand}`, item.variant && `옵션: ${item.variant}`, item.price && `가격: ${item.price}`, item.body?.slice(0,1600)].filter(Boolean).join('\n');
    ...
    await adapter.add({ kind: ['product','image','video','text','other'].includes(item.kind) ? item.kind : 'product', entry: 'capture', source_url: item.source_url, title: item.title.slice(0, 300), note: note || null }, files);
