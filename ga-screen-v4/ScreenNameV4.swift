// GA 통합 화면명 v4 — 생성 파일. screen-contract.v4.json에서 build_v4.py로 생성. 직접 수정하지 않습니다.
public enum ScreenNameV4: String, CaseIterable {
    public static let schemaVersion = 4
    /// [로그인] 앱 · 스플래시
    case app_splash = "app_splash"
    /// [로그인] 앱 · 점검 안내
    case app_maintenance = "app_maintenance"
    /// [로그인] 로그인 · 방법 선택(가입 시작 포함)
    case auth_entry = "auth_entry"
    /// [회원가입] 가입 · 약관 동의
    case signup_terms = "signup_terms"
    /// [회원가입] 가입 · 인증번호 입력
    case signup_email_verify = "signup_email_verify"
    /// [회원가입] 가입 · 비밀번호 설정
    case signup_password = "signup_password"
    /// [로그인] 로그인 · 이메일 입력
    case login_email = "login_email"
    /// [로그인] 로그인 · 비밀번호 입력
    case login_password = "login_password"
    /// [로그인] 로그인 · 2단계 인증번호 입력
    case login_2fa_email = "login_2fa_email"
    /// [로그인] 비밀번호 재설정 · 인증번호 입력
    case reset_password_verify = "reset_password_verify"
    /// [로그인] 비밀번호 재설정 · 새 비밀번호 설정
    case reset_password_new = "reset_password_new"
    /// [회원가입] 가입 · ID 입력
    case signup_id = "signup_id"
    /// [회원가입] 가입 · 이름 입력(한글)
    case signup_name_ko = "signup_name_ko"
    /// [회원가입] 가입 직후 · 접근 권한 안내
    case welcome_permission = "welcome_permission"
    /// [회원가입] 가입 직후 · 튜토리얼
    case welcome_tutorial = "welcome_tutorial"
    /// [회원가입] 가입 직후 · 환영 영상
    case welcome_video = "welcome_video"
    /// [회원가입] 약관 · 개인정보 처리방침 재동의
    case agreement_reconsent = "agreement_reconsent"
    /// [회원가입] 약관 · 전문 보기(웹)
    case agreement_document = "agreement_document"
    /// [마이] 프로필 커버 · 등록(한 줄 소개·직업·배경·프로필 사진)
    case profile_cover_create = "profile_cover_create"
    /// [마이] 프로필 커버 · 수정
    case profile_cover_edit = "profile_cover_edit"
    /// [마이] 자기소개 · 만들기(커버 등록 뒤)
    case profile_intro_create = "profile_intro_create"
    /// [마이] 자기소개 · 수정
    case profile_intro_edit = "profile_intro_edit"
    /// [회원가입] MY · 프로필 커버 없음 상태
    case my_profile_intro = "my_profile_intro"
    /// [디스커버리] 디스커버리 · 피드
    case discovery_feed = "discovery_feed"
    /// [디스커버리] 디스커버리 · 추천 크리에이터 전체보기
    case discovery_creator_list = "discovery_creator_list"
    /// [디스커버리] 영감 · 목록(딥링크)
    case inspiration_list = "inspiration_list"
    /// [디스커버리] 포트폴리오 · 상세
    case portfolio_detail = "portfolio_detail"
    /// [디스커버리] 영감 · 상세
    case inspiration_detail = "inspiration_detail"
    /// [디스커버리] 포트폴리오 · 댓글
    case portfolio_comment_list = "portfolio_comment_list"
    /// [디스커버리] 영감 · 댓글
    case inspiration_comment_list = "inspiration_comment_list"
    /// [디스커버리] 리스펙트 · 함께 보낼 한마디 선택
    case respect_select = "respect_select"
    /// [디스커버리] 리스펙트 · 보낸 사람 목록
    case respect_list = "respect_list"
    /// [디스커버리] 첨부 · 이미지 전체 화면 보기(자기소개)
    case media_viewer = "media_viewer"
    /// [파인더] 파인더 · 홈
    case finder_main = "finder_main"
    /// [파인더] 파인더 · 검색 입력
    case finder_search = "finder_search"
    /// [파인더] 파인더 · 크리에이터 검색 결과
    case finder_result_creator = "finder_result_creator"
    /// [파인더] 파인더 · 포트폴리오 검색 결과
    case finder_result_portfolio = "finder_result_portfolio"
    /// [파인더] 파인더 · 프로젝트 검색 결과
    case finder_result_project = "finder_result_project"
    /// [파인더] 파인더 · 필터
    case finder_filter = "finder_filter"
    /// [파인더] 파인더 · Shuffle 메인(웹)
    case finder_shuffle_main = "finder_shuffle_main"
    /// [파인더] 파인더 · Shuffle 키워드 상세
    case finder_shuffle_keyword = "finder_shuffle_keyword"
    /// [채티] 채티 · 다이얼 홈(팔로잉 크리에이터 판)
    case dial_main = "dial_main"
    /// [디스커버리] 크리에이터 · 프로필
    case profile_detail = "profile_detail"
    /// [디스커버리] 크리에이터 · 관계 목록(일촌·팔로워·팔로잉)
    case relation_list = "relation_list"
    /// [디스커버리] 크리에이터 · 함께 아는 일촌
    case first_connection_mutual = "first_connection_mutual"
    /// [디스커버리] 노크 · 용건 선택
    case knock_type_select = "knock_type_select"
    /// [디스커버리] 채팅방 신고 · 사유 선택
    case report_reason = "report_reason"
    /// [디스커버리] 채팅방 신고 · 사유 직접 입력
    case report_detail_input = "report_detail_input"
    /// [마이] MY · 내 프로필
    case my_profile = "my_profile"
    /// [마이] MY · 프로필 미리보기(웹)
    case my_profile_preview = "my_profile_preview"
    /// [마이] MY · 오늘 방문자
    case my_profile_visitors = "my_profile_visitors"
    /// [마이] MY · 노크 수신 설정
    case my_knock_settings = "my_knock_settings"
    /// [마이] 내 정보 · 관리 홈(기본 정보 관리)
    case my_info_main = "my_info_main"
    /// [마이] 내 정보 · ID 변경
    case my_info_id = "my_info_id"
    /// [마이] 내 정보 · 한글 이름
    case my_info_name_ko = "my_info_name_ko"
    /// [마이] 내 정보 · 영문 이름
    case my_info_name_en = "my_info_name_en"
    /// [마이] 내 정보 · 직업 관리
    case my_info_profession = "my_info_profession"
    /// [마이] 내 정보 · 직업 선택
    case my_info_profession_select = "my_info_profession_select"
    /// [마이] 내 정보 · 직업 직접 추가
    case my_info_profession_direct = "my_info_profession_direct"
    /// [마이] 내 정보 · 활동 지역 관리
    case my_info_region = "my_info_region"
    /// [마이] 내 정보 · 해외 도시 입력
    case my_info_region_global_city = "my_info_region_global_city"
    /// [마이] 내 정보 · 연락용 이메일 입력
    case my_info_email_register = "my_info_email_register"
    /// [마이] 내 정보 · 연락용 이메일 인증번호 입력
    case my_info_email_verify = "my_info_email_verify"
    /// [마이] 내 정보 · 전화번호 입력
    case my_info_phone_register = "my_info_phone_register"
    /// [마이] 내 정보 · 링크 입력
    case my_info_link_register = "my_info_link_register"
    /// [마이] 내 정보 · 생년월일·성별
    case my_info_birth_gender = "my_info_birth_gender"
    /// [마이] 내 정보 · 항목별 공개 범위
    case my_info_visibility = "my_info_visibility"
    /// [마이] 앱 · 계정 이메일 필수 등록
    case account_email_required = "account_email_required"
    /// [마이] 커리어 · 관리 홈
    case resume_main = "resume_main"
    /// [마이] 커리어 · 경력 입력
    case resume_career_form = "resume_career_form"
    /// [마이] 커리어 · 학력 입력
    case resume_education_form = "resume_education_form"
    /// [마이] 커리어 · 자격·수상 입력
    case resume_awards_form = "resume_awards_form"
    /// [마이] 첨부 · 이미지 편집(자르기·회전)
    case media_image_edit = "media_image_edit"
    /// [마이] 첨부 · 동영상 편집(구간 자르기)
    case media_video_edit = "media_video_edit"
    /// [마이] 포트폴리오 · 작성
    case portfolio_create = "portfolio_create"
    /// [마이] 포트폴리오 · 수정
    case portfolio_edit = "portfolio_edit"
    /// [마이] 프로젝트 · 작성
    case project_create = "project_create"
    /// [마이] 프로젝트 · 수정
    case project_edit = "project_edit"
    /// [마이] 영감 · 작성
    case inspiration_create = "inspiration_create"
    /// [마이] 영감 · 수정
    case inspiration_edit = "inspiration_edit"
    /// [채티] 채티 · 1:1 대화방
    case chat_room_direct = "chat_room_direct"
    /// [채티] 채티 · 라운지 대화방
    case chat_room_lounge = "chat_room_lounge"
    /// [채티] 채티 · 프로젝트 대화방
    case chat_room_project = "chat_room_project"
    /// [채티] 채티 · 팀 대화방
    case chat_room_team = "chat_room_team"
    /// [채티] 채티 · 대화방 메뉴
    case chat_room_menu = "chat_room_menu"
    /// [채티] 라운지 · 만들기
    case lounge_create = "lounge_create"
    /// [채티] 라운지 · 수정
    case lounge_edit = "lounge_edit"
    /// [채티] 알림함 · 받은 알림 목록
    case notification_center = "notification_center"
    /// [채티] 인맥 관리 · 일촌
    case contacts_first_connections = "contacts_first_connections"
    /// [채티] 인맥 관리 · 팔로워
    case contacts_follower = "contacts_follower"
    /// [채티] 인맥 관리 · 팔로잉
    case contacts_following = "contacts_following"
    /// [채티] 인맥 관리 · 검색 결과
    case contacts_search_results = "contacts_search_results"
    /// [디스커버리] 프로젝트 · 상세
    case project_detail = "project_detail"
    /// [디스커버리] 프로젝트 카드 · ID 목록(딥링크)
    case project_list = "project_list"
    /// [디스커버리] 프로젝트 · 지원 작성
    case project_application_compose = "project_application_compose"
    /// [디스커버리] 프로젝트 · 지원자 목록
    case project_applicants = "project_applicants"
    /// [채티] 협업 제안 · 작성(웹)
    case proposal_create = "proposal_create"
    /// [채티] 협업 제안 · 수정(웹)
    case proposal_edit = "proposal_edit"
    /// [채티] 팀 · 목록(내 팀과 받은 초대)
    case team_list = "team_list"
    /// [채티] 팀 · 상세
    case team_detail = "team_detail"
    /// [채티] 팀 · 만들기
    case team_create = "team_create"
    /// [채티] 팀 · 수정
    case team_edit = "team_edit"
    /// [채티] 팀 · 멤버 초대
    case team_invite = "team_invite"
    /// [설정] 설정 · 홈
    case settings_main = "settings_main"
    /// [설정] 설정 · 계정
    case settings_account = "settings_account"
    /// [설정] 설정 · 알림 수신 설정
    case settings_notification = "settings_notification"
    /// [설정] 설정 · 공개 범위 메뉴
    case settings_visibility = "settings_visibility"
    /// [설정] 설정 · 프로필 공개 범위
    case settings_visibility_profile = "settings_visibility_profile"
    /// [설정] 설정 · 차단한 사용자
    case settings_blocked_members = "settings_blocked_members"
    /// [설정] 설정 · 비밀번호 변경
    case settings_change_password = "settings_change_password"
    /// [설정] 설정 · 2단계 인증
    case settings_2fa = "settings_2fa"
    /// [설정] 설정 · 2단계 인증 이메일 관리
    case settings_2fa_email = "settings_2fa_email"
    /// [설정] 설정 · 2단계 인증번호 입력(인증 켜기)
    case settings_2fa_email_verify = "settings_2fa_email_verify"
    /// [설정] 회원 탈퇴 · 비밀번호 확인
    case account_delete_password = "account_delete_password"
    /// [설정] 회원 탈퇴 · 사유 선택
    case account_delete_reason = "account_delete_reason"
    /// [설정] 회원 탈퇴 · 최종 확인
    case account_delete_confirm = "account_delete_confirm"
    /// [설정] 고객센터 · 홈
    case support_main = "support_main"
    /// [설정] 설정 · 서비스 동의(약관 목록)
    case settings_agreements = "settings_agreements"
    /// [설정] Muse 특권 안내(웹)
    case settings_muse_privileges = "settings_muse_privileges"
    /// [설정] 내 활동 · 홈
    case settings_contents = "settings_contents"
    /// [설정] 내 활동 · 댓글과 답글
    case settings_contents_comments = "settings_contents_comments"
    /// [설정] 내 활동 · 리스펙트한 포트폴리오
    case settings_contents_respected = "settings_contents_respected"
    /// [설정] 내 활동 · 좋아요한 콘텐츠
    case settings_contents_liked = "settings_contents_liked"
    /// [설정] 북마크 · 크리에이터
    case bookmark_creator = "bookmark_creator"
    /// [설정] 북마크 · 포트폴리오
    case bookmark_portfolio = "bookmark_portfolio"
    /// [디스커버리] 아케이드 · 카드 보스전
    case arcade_card_boss = "arcade_card_boss"
    /// [디스커버리] 아케이드 · 블록깨기
    case arcade_breakout = "arcade_breakout"
    /// [로그인] 앱 · 필수 업데이트
    case app_update_required = "app_update_required"
    /// [로그인] 앱 · 로그인 만료 안내
    case auth_session_expired = "auth_session_expired"
    /// [마이] 프로필 커버 · 등록 필요(업로드 진입 시)
    case profile_cover_required = "profile_cover_required"
    /// [채티] 채티 · 대화 목록
    case chat_list = "chat_list"
    /// [채티] 채티 · 라운지 목록
    case lounge_list = "lounge_list"
    /// [채티] 채티 · 프로젝트 현황(참여·지원·새로 열림)
    case project_chat_list = "project_chat_list"
    /// [설정] 회원 탈퇴 · 사유 직접 입력
    case account_delete_reason_input = "account_delete_reason_input"
    /// [설정] 설정 · 개인정보 이용 동의(딥링크)
    case settings_personal_info_consent = "settings_personal_info_consent"
    /// [설정] 설정 · 개인정보 이용 동의 철회(딥링크)
    case settings_personal_info_withdraw = "settings_personal_info_withdraw"
    /// [마이] 회원 자격 · 베타 혜택 안내(웹)
    case member_privileges = "member_privileges"
    /// [채티] 채티 · 다이얼 안내(팔로잉 8명 미만)
    case dial_guide = "dial_guide"
    /// [채티] 채티 · 다이얼 파도타기(크리에이터 판)
    case dial_creator = "dial_creator"
    /// [마이] 프로필 커버 · 디자인 선택(웹)
    case profile_cover_design = "profile_cover_design"
}

/// 1depth 구분값(content_group·screen_class). nil이면 화면이 뜬 순간의 활성 하단 탭 코드를 넣는다.
public enum ScreenGroupV4 {
    public static let values = ["login", "signup", "discovery", "finder", "chatty", "my", "settings"]
    public static let of: [ScreenNameV4: String?] = [
        .app_splash: "login",
        .app_maintenance: "login",
        .auth_entry: "login",
        .signup_terms: "signup",
        .signup_email_verify: "signup",
        .signup_password: "signup",
        .login_email: "login",
        .login_password: "login",
        .login_2fa_email: "login",
        .reset_password_verify: "login",
        .reset_password_new: "login",
        .signup_id: "signup",
        .signup_name_ko: "signup",
        .welcome_permission: "signup",
        .welcome_tutorial: "signup",
        .welcome_video: "signup",
        .agreement_reconsent: "signup",
        .agreement_document: "signup",
        .profile_cover_create: "my",
        .profile_cover_edit: "my",
        .profile_intro_create: "my",
        .profile_intro_edit: "my",
        .my_profile_intro: "signup",
        .discovery_feed: "discovery",
        .discovery_creator_list: "discovery",
        .inspiration_list: nil,
        .portfolio_detail: nil,
        .inspiration_detail: nil,
        .portfolio_comment_list: nil,
        .inspiration_comment_list: nil,
        .respect_select: "discovery",
        .respect_list: nil,
        .media_viewer: nil,
        .finder_main: "finder",
        .finder_search: "finder",
        .finder_result_creator: "finder",
        .finder_result_portfolio: "finder",
        .finder_result_project: "finder",
        .finder_filter: "finder",
        .finder_shuffle_main: "finder",
        .finder_shuffle_keyword: "finder",
        .dial_main: "chatty",
        .profile_detail: nil,
        .relation_list: nil,
        .first_connection_mutual: "discovery",
        .knock_type_select: "discovery",
        .report_reason: "discovery",
        .report_detail_input: "discovery",
        .my_profile: "my",
        .my_profile_preview: "my",
        .my_profile_visitors: "my",
        .my_knock_settings: "my",
        .my_info_main: "my",
        .my_info_id: "my",
        .my_info_name_ko: "my",
        .my_info_name_en: "my",
        .my_info_profession: "my",
        .my_info_profession_select: "my",
        .my_info_profession_direct: "my",
        .my_info_region: "my",
        .my_info_region_global_city: "my",
        .my_info_email_register: "my",
        .my_info_email_verify: "my",
        .my_info_phone_register: "my",
        .my_info_link_register: "my",
        .my_info_birth_gender: "my",
        .my_info_visibility: "my",
        .account_email_required: "my",
        .resume_main: "my",
        .resume_career_form: "my",
        .resume_education_form: "my",
        .resume_awards_form: "my",
        .media_image_edit: "my",
        .media_video_edit: "my",
        .portfolio_create: "my",
        .portfolio_edit: "my",
        .project_create: "my",
        .project_edit: "my",
        .inspiration_create: "my",
        .inspiration_edit: "my",
        .chat_room_direct: "chatty",
        .chat_room_lounge: "chatty",
        .chat_room_project: "chatty",
        .chat_room_team: "chatty",
        .chat_room_menu: "chatty",
        .lounge_create: "chatty",
        .lounge_edit: "chatty",
        .notification_center: "chatty",
        .contacts_first_connections: "chatty",
        .contacts_follower: "chatty",
        .contacts_following: "chatty",
        .contacts_search_results: "chatty",
        .project_detail: nil,
        .project_list: nil,
        .project_application_compose: nil,
        .project_applicants: "discovery",
        .proposal_create: "chatty",
        .proposal_edit: "chatty",
        .team_list: "chatty",
        .team_detail: nil,
        .team_create: "chatty",
        .team_edit: "chatty",
        .team_invite: "chatty",
        .settings_main: "settings",
        .settings_account: "settings",
        .settings_notification: "settings",
        .settings_visibility: "settings",
        .settings_visibility_profile: "settings",
        .settings_blocked_members: "settings",
        .settings_change_password: "settings",
        .settings_2fa: "settings",
        .settings_2fa_email: "settings",
        .settings_2fa_email_verify: "settings",
        .account_delete_password: "settings",
        .account_delete_reason: "settings",
        .account_delete_confirm: "settings",
        .support_main: "settings",
        .settings_agreements: "settings",
        .settings_muse_privileges: "settings",
        .settings_contents: "settings",
        .settings_contents_comments: "settings",
        .settings_contents_respected: "settings",
        .settings_contents_liked: "settings",
        .bookmark_creator: "settings",
        .bookmark_portfolio: "settings",
        .arcade_card_boss: "discovery",
        .arcade_breakout: "discovery",
        .app_update_required: "login",
        .auth_session_expired: "login",
        .profile_cover_required: "my",
        .chat_list: "chatty",
        .lounge_list: "chatty",
        .project_chat_list: "chatty",
        .account_delete_reason_input: "settings",
        .settings_personal_info_consent: "settings",
        .settings_personal_info_withdraw: "settings",
        .member_privileges: "my",
        .dial_guide: "chatty",
        .dial_creator: "chatty",
        .profile_cover_design: "my",
    ]
}
